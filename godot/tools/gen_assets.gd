...
func _draw_spirit(name: String, body: Color, accent: Color, shape: int) -> void:
	...
	_rect(image, 12, 54, 40, 3, STONE_DARK)
	if shape == 1: # Sprout: round seedling with leaf ears.
		_rect(image, 18, 25, 28, 28, INK)
		_rect(image, 21, 27, 22, 23, body)
		_rect(image, 14, 17, 16, 11, INK)
		_rect(image, 17, 19, 12, 7, accent)
		_rect(image, 35, 14, 15, 13, INK)
		_rect(image, 36, 17, 11, 8, accent)
	elif shape == 2: # Ember fox: long body, bright tail, pointed ears.
		_rect(image, 10, 37, 43, 15, INK)
		_rect(image, 14, 39, 35, 11, body)
		_rect(image, 37, 20, 19, 24, INK)
		_rect(image, 40, 23, 13, 18, body)
		_rect(image, 38, 14, 7, 12, INK)
		_rect(image, 49, 13, 7, 13, INK)
		_rect(image, 5, 27, 15, 9, INK)
		_rect(image, 7, 29, 13, 5, accent)
	else: # Gale spirit: floating teardrop with fin-like arms.
		_rect(image, 19, 17, 27, 34, INK)
		_rect(image, 22, 20, 21, 27, body)
		_rect(image, 10, 29, 13, 9, INK)
		_rect(image, 12, 31, 10, 5, accent)
		_rect(image, 43, 29, 12, 9, INK)
		_rect(image, 43, 31, 9, 5, accent)
		_rect(image, 27, 47, 11, 7, accent)
	_pixel(image, 28, 33, PARCHMENT)
	_pixel(image, 37, 33, PARCHMENT)
	_pixel(image, 28, 34, INK)
	_pixel(image, 37, 34, INK)
	_save(image, "char_%s.png" % name)
...
func _write_manifest() -> void:
	...
	text += "| `game/ui/ui_sounds.gd` (procedural) | authored; synthesized tones at runtime, no audio file | CC0 / public-domain (self-authored) | 2026-07-10 | DS-style UI blips; no external audio asset. |\n"
